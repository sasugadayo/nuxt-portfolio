...
<script>
export default {
  async asyncData({ $microcms, route, params }) {
    const work = await $microcms.get({
      endpoint: `works/${route.params.id}`,
    })
    return {
      work,
    }
  ...
  head() {
    ...
    this.meta.title = this.work.title
    this.meta.description = this.work.overview
    this.meta.type = 'article'
    this.meta.url = this.base + path
    this.meta.image = this.work.thumbnail
